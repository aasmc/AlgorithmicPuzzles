...
class SubstringConcatenationSolutionTest {
    ...
    fun testPermutations() {
        val input = arrayOf("c","b","a","c","a","a","a","b","c")
        val sequence = sut.permutations(input)
        val correctPermutations = hashSetOf<String>(
            "aabbccaca",
            "babccacaa",
            "abccacaab",
            "ccacaabab",
        ...
        sequence.forEach { permutation ->
            if (permutation in correctPermutations) {
                println(permutation)
            }
        }
    }
